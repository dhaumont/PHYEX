!     ######spl
         ...
         USE YOMHOOK , ONLY : LHOOK, DR_HOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK, JPHOOK
!        ###################
!
!!
!!    PURPOSE
...
CONTAINS
  ...
  SUBROUTINE DUSTOPT_GET(     &
    ...
    !LOCAL VARIABLES
    REAL, DIMENSION(SIZE(PSVT,1),SIZE(PSVT,2),SIZE(PSVT,3)) :: ZSVT 
    REAL, DIMENSION(SIZE(PSVT,1),SIZE(PSVT,2), NMODE_DST) :: ZMASS         ![kg/m3] mass of one dust mode
    REAL, DIMENSION(SIZE(PSVT,1),SIZE(PSVT,2), NMODE_DST) :: ZRADIUS       ![um] number median radius of one dust mode
    REAL, DIMENSION(SIZE(PSVT,1),SIZE(PSVT,2), NMODE_DST) :: ZSIGMA        ![-] dispersion coefficient one dust mode
    ...
    ALLOCATE(ZTAU550_MDE(SIZE(PTAU550,1),SIZE(PTAU550,2),NMODE_DST)) 
    ALLOCATE(ZTAU_WVL_MDE(SIZE(PTAU550,1),SIZE(PTAU550,2),KSWB,NMODE_DST))
    ALLOCATE(ZPIZA_WVL_MDE(SIZE(PTAU550,1),SIZE(PTAU550,2),KSWB,NMODE_DST))
    ALLOCATE(ZCGA_WVL_MDE(SIZE(PTAU550,1),SIZE(PTAU550,2),KSWB,NMODE_DST))
  ...
  SUBROUTINE DUSTOPT_LKT(              &
    ...
    !LOCALS
    REAL, DIMENSION(SIZE(PTAU550,1),SIZE(PTAU550,2),KSWB) :: ZEXT_COEFF_WVL    ![m2/kg] Extinction coefficient at wvl
    REAL, DIMENSION(SIZE(PTAU550,1),SIZE(PTAU550,2)     ) :: ZEXT_COEFF_550    ![m2/kg] Extinction coefficient at 550nm
    ...
    INTEGER                   :: JI, JK  ![idx] counters for lon, lat and lev
    INTEGER                   :: RG_IDX      ![idx] index for radius to get in look up table
    INTEGER                   :: SG_IDX      ![idx] index for sigma to get in look up table
    REAL,DIMENSION(SIZE(PRG,1),SIZE(PRG,2)) :: ZRG     ![um] bounded value for number median radius
    REAL,DIMENSION(SIZE(PRG,1),SIZE(PRG,2)) :: ZSIGMA  ![um] bounded value for sigma
    ...
    DO WVL_IDX = 1,KSWB
       DO JK=2,SIZE(PMASS,2)
          JKRAD = JK - 1  !Index in radiation code
             DO JI=1,SIZE(PMASS,1)

                !Get the correct indexes for the look up tables
                RG_IDX =  nint(                                  &
                     log(ZRG(JI,JK)/XRADIUS_LKT_MIN)          &
                     *FACT_RADIUS                                &
                     +0.5)
                   
                SG_IDX = nint((ZSIGMA(JI,JK)-XSIGMA_LKT_MIN)*FACT_SIGMA + 0.5) 

                !Open the look up tables and get the right values out of them
                !The extinction coefficient for 
                ZEXT_COEFF_WVL(JI,JK,WVL_IDX)         = XEXT_COEFF_WVL_LKT(RG_IDX,SG_IDX,WVL_IDX)
                ZEXT_COEFF_550(JI,JK)                 = XEXT_COEFF_550_LKT(RG_IDX,SG_IDX)
                !Switch to radiation code indexes for the output values
                PPIZA_WVL(JI,JKRAD,WVL_IDX)              = XPIZA_LKT(RG_IDX,SG_IDX,WVL_IDX)
                PCGA_WVL(JI,JKRAD,WVL_IDX)               = XCGA_LKT(RG_IDX,SG_IDX,WVL_IDX)
             ENDDO
       ENDDO
    ENDDO

    !Get the optical depth of this mode using the looked up extinction coeffient
    DO JK=2,SIZE(PZZ,2)-1
       JKRAD = JK - 1           !Index in radiation code
