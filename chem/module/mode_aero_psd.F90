...
USE YOMHOOK , ONLY : LHOOK, DR_HOOK, JPHOOK, JPHOOK, JPHOOK
!!   ########################
!!
...
REAL,DIMENSION(SIZE(PSVT,1), SIZE(PSVT,2),SIZE(PSVT,3)) :: ZSV ! [aerosol concentration]
REAL,DIMENSION(SIZE(PSVT,1), SIZE(PSVT,2)) :: ZSIGMA   ! [-] standard deviation
REAL,DIMENSION(SIZE(PSVT,1),SIZE(PSVT,2),NSP+NCARB+NSOA,JPMODE):: ZCTOTA
REAL,DIMENSION(SIZE(PSVT,1),SIZE(PSVT,2),JPMODE*3) :: ZM
...
ALLOCATE (ZM(SIZE(PSVT,1), SIZE(PSVT,2), JPMODE*3))
