!     ######spl
      SUBROUTINE  INV_LEVELS(KIDIA,KFDIA,KLON,KLEV,KINCR,PARRAYALD,PARRAYMNH)
...
REAL, DIMENSION(KLON,KLEV),   INTENT(INOUT) :: PARRAYALD  ! Aladin type arry
REAL, DIMENSION(KLON,KLEV+2), INTENT(INOUT) :: PARRAYMNH  ! MNH type array
...
IF (KINCR==1) THEN
       ...
       DO JLEV = 2 , KLEV+1
           DO JLON = KIDIA,KFDIA
              PARRAYMNH(JLON,JLEV)= PARRAYALD(JLON,KLEV+2-JLEV)
           ENDDO
       ENDDO
       DO JLON = KIDIA,KFDIA
           PARRAYMNH(JLON,1)= PARRAYMNH(JLON,2)
           PARRAYMNH(JLON,KLEV+2)= PARRAYMNH(JLON,KLEV+1)
       ENDDO
...
ElSE IF  (KINCR==-1) THEN
       DO JLEV = 1 ,KLEV
            DO JLON = KIDIA,KFDIA
              PARRAYALD(JLON,JLEV)=PARRAYMNH(JLON,KLEV+2-JLEV)
            ENDDO
       ENDDO
